...
Gtd.prototype = {
    ...
    updateProcessList: function (sCategoryId) {
        ...
        this.each(keyArr, function (i, keyValue) {
            var taskTimeHtml =  '<h3 class="taskTime">' + keyValue + '</h3>',
                taskTitleHtml = '',
                finishedState = '',
                sSelectClass = '';
            this.each(oProcessData[keyValue], function (j, task) {
                ...
                taskTitleHtml += '<li class="noteTitle' + sSelectClass + ' ' + finishedState + '" task-id='+ task.taskId +' ><a href="#">' + task.taskTitle + '</a></li>';
            });
                ...
                if (_this.hasClass(oTarget.parentNode.parentNode, 'task-dft')) { // 默认分类不能删除
                    ...
                    _this.oConfirmBoxBtnSure.onclick = function () {
                        ...
                        if (!_this.hasClass(oTarget.parentNode, 'ico-task-sec')) { // 删除主目录
                            ...
                            _this.each(oDeleteTaskList.rows, function (i, delTask) {
                                _this.each(oTaskList.rows, function(j, task) {
                                    if (task.taskId === delTask) {
                                        log("删除了id为：" + task.taskId + '的任务');
                                        oTaskList.rows.splice(j, 1);
                                    }
                                });
                            });
                            _this.each(oSaveStructorData.rows, function (i, value) {
                                if (value.id === sId) {
                                    var sSubcategory = value.subcategory;
                                    if (sSubcategory) {
                                        _this.each(sSubcategory, function (j, val) {
                                            log('delete 子分类 id' + val.id);
                                            delete _this.ls[val.id];
                                        });
                                    }
                                }
                            });
                            ...
                            _this.each(oSaveStructorData.rows, function (i, value) {
                                if (value.id === sParentId) {
                                    _this.each(value.subcategory, function(j, val) {
                                        if (val.id === sId) {
    ...
    addTask: function () {
        ...
        oOkBtn.onclick = function () {
            ...
            var aIdArray = [];
            _this.each(oTaskList.rows, function (i, value) {
